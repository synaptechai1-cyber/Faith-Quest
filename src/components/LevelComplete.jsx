...
export default function LevelComplete({ level, xpEarned, totalXp, onNext, isGameComplete, difficulty }) {
  const [show, setShow] = useState(false)
  const nextLevel = LEVELS.find(l => l.id === level.id + 1)
  const diffBonus = difficulty.id === 'expert' ? '2x' : difficulty.id === 'intermediate' ? '1.5x' : '1x'
      ...
      <div className={`max-w-lg w-full text-center transition-all duration-700 ${show ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        <p className="text-slate-400 text-sm mb-8">
          {isGameComplete
            ...
            : "You've overcome the challenges of this chapter. Your faith grows stronger."}
        </p>

        {/* XP earned */}
        <div className="game-card-gold p-6 mb-4 anim-pop">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <p className="font-display text-xs text-slate-400 uppercase tracking-widest mb-1">Multiplier</p>
              <p className="font-display font-bold text-3xl" style={{color: difficulty.color}}>{diffBonus}</p>
            </div>
            <div>
        ...
        {/* Runner game teaser */}
        <div className="game-card p-5 mb-5 border-yellow-500/20">
          <div className="flex items-center gap-3">
            <span className="text-3xl">🎮</span>
            <div className="text-left">
              <p className="font-display font-bold text-white text-sm">Bonus Stage Unlocked!</p>
              <p className="text-xs text-slate-400 mt-0.5">Play Faith Runner — collect scrolls, dodge temptations, earn bonus XP!</p>
            </div>
          </div>
        </div>
